...
namespace Puzzle_2
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            int tripOne = trip(grid, startPosition, endPosition, 0, row, col);
            int tripTwo = trip(grid, endPosition, startPosition, tripOne, row, col);
            int tripThree = trip(grid, startPosition, endPosition, tripTwo, row, col);

            Console.WriteLine(tripThree);

            reader.Close();
            reader.Dispose();
        }

        public static int trip(char[,] grid, (int, int) startPosition, (int, int) endPosition, int step, int row, int col)
        {
            Queue<PathDto> queue = new Queue<PathDto>();

            queue.Enqueue(new PathDto(startPosition.Item1, startPosition.Item2, step));
            ...
            while (queue.Count != 0)
            {
                ...
                if (queue.Where(q => q.x == currPosition.Item1 && q.y == currPosition.Item2 + 1 && q.step == p.step + 1).Any() == false && (currPosition.Item2 < row - 2 || (currPosition.Item1 == endPosition.Item1 && currPosition.Item2 < row - 1)) && anyBlizzardTargeting(grid, (currPosition.Item1, currPosition.Item2 + 1), p.step, row, col) == false)
                {
                    queue.Enqueue(new PathDto(currPosition.Item1, currPosition.Item2 + 1, p.step + 1));
                }
                if (queue.Where(q => q.x == currPosition.Item1 && q.y == currPosition.Item2 - 1 && q.step == p.step + 1).Any() == false && (currPosition.Item2 > 1 || (currPosition.Item1 == endPosition.Item1 && currPosition.Item2 > 0)) && anyBlizzardTargeting(grid, (currPosition.Item1, currPosition.Item2 - 1), p.step, row, col) == false)
                {
                    queue.Enqueue(new PathDto(currPosition.Item1, currPosition.Item2 - 1, p.step + 1));
                }
