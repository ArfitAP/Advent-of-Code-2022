...
int64_t nthElement(struct Number* sequence, int numOfNumbers, int n);
...
int main()
{
	...
	while (fgets(line, sizeof(line), file)) {
        ...
        sequence[index].value = atoi(line) * 811589153LL;  
...
void move(struct Number* sequence, int numOfNumbers, int orderNumber)
{
	...
	for(i = 0; i < numOfNumbers; i++)
	{
		if(sequence[i].order == orderNumber)
		{
			...
			{
				int n = 0;
				int numOfSwaps = (-sequence[i].value) % (numOfNumbers - 1);
				int ind1 = i;
